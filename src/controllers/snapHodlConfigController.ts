...
function sortStakingContractData(data: any[]) {
  return data.sort((a, b) =>
    a.stakingPoolName.localeCompare(b.stakingPoolName)
  );
}
...
export const retrieveSnapHodlConfigs = async (): Promise<SnapHodlConfig[]> => {
  try {
    ...
    if (err instanceof Error) {
      ...
      throw new Error(
        "An error occurred when attempting to fetch SnapHodlConfigs"
      );
    }
  }
};
...
export const getSnapHodlConfigs = async (req: Request, res: Response) => {
  try {
    ...
    if (err instanceof Error) {
      ...
      res.status(500).json({
        message:
          "An error occurred when attempting to request SnapHodlConfigs to be retrieved",
      });
    }
  }
};
...
export const createSnapHodlConfig = async (req: Request, res: Response) => {
  ...
  const lowerCaseStakingContractData =
    toLowerCaseStakingContractData(stakingContractData);

  // Sort stakingContractData by stakingPoolName for consistent comparison
  const sortedStakingContractData = sortStakingContractData(
    lowerCaseStakingContractData
  );
...
export const getSnapShotBySnapShotIdAndAddress = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const snapHodlConfigBalance = await SnapHodlConfigBalanceModel.findOne({
      snapHodlConfigId: snapShotId,
    });
    ...
    const snapShotBalance =
      snapHodlConfigBalance.totalStakedBalance.get(address);
...
export const getAllSnapShots = async (req: Request, res: Response) => {
  try {
    ...
    const snapHodlConfigBalances = await SnapHodlConfigBalanceModel.find(query)
      .skip(skip)
      .limit(limit);
